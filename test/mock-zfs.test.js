...
function checkPoolList(t, have, want) {
	...
	for (val of s2) {
		t.ok(s1.delete(val), sprintf('%j was found in have array',
			val));
	}
	for (val of s1) {
...
tap.test('zpool', function (tt) {
	tt.afterEach(function (done) {
		mockds.reset();
		done();
	});

	tt.test('zpool', function (t) {
		...
		zpool.create('pool1', {}, function (err) {
			t.notEqual(err || null, null,
				'zpool create pool1 should fail when it ' +
				'already exists');
			var msg = err ? err.message : '<no error>';
			t.match(msg, /pool already exists/,
		...
		zpool.status('pool2', function (err, val) {
			t.error(err,
				'zpool status pool2 does not have an error');
			t.equal(val, 'ONLINE',
				'pool2 is online after creation');
		});
		...
		zpool.list('pool2', { fields: [ 'name' ] },
			...
			t.error(err,
				'zpool list pool2 does not return an error');
		...
		zpool.destroy('pool1', function (err) {
			t.error(err,
				'zpool destroy pool1 does not return an error');
		});
	...
	tt.test('zfs', function (t) {
		zfs.create('nosuchpool', function (err) {
			t.notEqual(err || null, null,
				'cannot create a top-level dataset');
			var msg = err ? err.message : '<no error>';
		...
		zfs.rename('testpool/foo/bar', 'testpool/baz', function (err) {
			t.error(err,
				'zfs rename testpool/foo/bar testpool/baz');
		});
		...
		zfs.get('testpool/baz', [ 'name', 'mountpoint' ], true,
			...
			t.equal(values[0][0], 'testpool/baz',
				'row 1 col 1 has dsname');
			t.equal(values[0][1], 'name',
				'row 1 col 2 has propname');
			t.equal(values[0][2], 'testpool/baz',
				'row 1 col 3 has value');
			t.equal(values[1][0], 'testpool/baz',
				'row 2 col 1 has dsname');
			t.equal(values[1][1], 'mountpoint',
				'row 2 col 2 has propname');
			t.equal(values[1][2], '/testpool/baz',
				'row 2 col 3 has value');
		});
		...
		zfs.get('testpool/baz', [ 'mountpoint' ], true,
			...
			t.equal(values[0][0], 'testpool/baz',
				'row 1 col 1 has dsname');
			t.equal(values[0][1], 'mountpoint',
				'row 1 col 2 has propname');
			t.equal(values[0][2], '/baz',
				'row 1 col 3 has value');
		});

		zfs.rename('testpool/baz', 'testpool/foo/blah', function (err) {
			t.error(err,
				'zfs rename testpool/baz testpool/foo/blah');
		});
		...
		zfs.list_snapshots('testpool/foo@snap1', function (err, fields,
		    values) {
			t.notEqual(err || null, null,
				'"zfs list_snapshots (2 args) not fully ' +
				'implemented');
			t.match(err ? err.message : '<no error>',
				/not implemented/,
		...
		zfs.list_snapshots(function (err, fields, values) {
			t.notEqual(err || null, null,
				'"zfs list_snapshots (1 arg) not fully ' +
				'implemented');
			t.match(err ? err.message : '<no error>',
				/not implemented/,
				'error message contains "not implemented"');
		...
		zfs.clone('testpool/foo@snap1', 'testpool/bar', function (err) {
			t.error(err,
				'zfs clone testpool/foo@snap1 testpool/bar');
		});
		...
		zfs.destroy('testpool/bar@snap2', function (err) {
			t.notEqual(err || null, null,
				'"zfs destroy testpool/bar@snap2" should ' +
				'fail due to hold');
			t.match(err ? err.message : '<no error>',
				/dataset is busy/,
		...
		zfs.destroyAll('testpool/bar@snap2', function (err) {
			t.notEqual(err || null, null,
				'"zfs destroy -r testpool/bar@snap2" should ' +
				'fail due to hold');
			t.match(err ? err.message : '<no error>',
				/dataset is busy/,
				'error message contains "dataset is busy"');
		...
		zfs.releaseHold('testpool/bar@snap2', 'something',
		    function (err) {
			t.error(err,
				'zfs release something testpool/bar@snap2');
		});
		...
		zfs.destroyAll('testpool/foo', function (err) {
			t.notEqual(err || null, null,
				'"zfs destroy -r testpool/foo" should fail ' +
				'due to clone');
			t.match(err ? err.message : '<no error>',
		...
		zfs.destroy('testpool/foo', function (err) {
			...
			t.notEqual(err || null, null,
				'"zfs destroy testpool/foo" should fail ' +
				'due to snapshot');
			t.match(err ? err.message : '<no error>',
			/has children/,
