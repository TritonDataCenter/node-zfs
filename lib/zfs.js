...
zpool.list = function () {
  ...
  var args = ['list', '-Hp', '-o', zpool.listFields_.join(',')];
...
zfs.get = function (name, propNames, callback) {
  ...
  execFile(exports.paths.zfs,
    ['get', '-Hp', '-o', 'source,property,value', propNames.join(','), name],
    { timeout: timeoutDuration },
    function (error, stdout, stderr) {
      if (error) {
        return callback(zfsError(error, stderr));
...
zfs.list = function () {
  ...
  var args = ['list', '-Hp', '-o', zfs.listFields_.join(','), '-t', options.type ];
