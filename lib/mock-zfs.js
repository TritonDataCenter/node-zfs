...
zpool.destroy = function (pool, callback) {
	...
	try {
		...
		callback(zfsError(err, 'cannot open \'' + pool + '\': no ' +
		    'such pool'));
		return;
	}
	callback();
...
zfs.create = function (name, callback) {
	...
	try {
		...
		if (err.name === 'DatasetExistsError') {
			err = zfsError(err,
			    'cannot create \'' + name + '\': dataset already '+
			    'exists');
		}
		callback(err);
		return;
...
zfs.snapshot = function (name, callback) {
	...
	if (!snapname) {
		callback(zfsError(null, 'cannot create snapshot \'' + name +
		    '\': empty component or misplaced \'@\' or \'#\' ' +
		    'delimiter in name'));
		return;
	}
	...
	try {
		...
		if (err.name === 'DatasetExistsError') {
			err = zfsError(err, 'cannot create snapshot \'' +
...
zfs.clone = function () {
	...
	if (ssds === null) {
		callback(zfsError(null,
		    'cannot open \'' + snapshot + '\': dataset does not ' +
		    'exist'));
		return;
	}
	try {
		...
		switch (err.name) {
			case 'SnapshotHoldError':
				err = new zfsError(err,
				    sprintf('cannot destroy \'%s\': dataset ' +
				    'is busy', name));
				break;
			case 'DescendantError':
				msg = sprintf('cannot destroy \'%s\': %s has ' +
				    'children', name, ds.type);
				let kids = [];
		    ...
		    case 'DependantError':
				msg = sprintf(
					'cannot destroy \'%s\': snapshot has ' +
					'dependent clones', name);
				msg += '\nuse \'-R\' to destroy the following ';
				msg += 'datasets:\n';
...
zfs.list = function () {
	...
	if (!parseable) {
		callback(notImplemented('zfs.list',
		    'parseable=false not implemented'));
		return;
	}
...
zfs.rename = function (name, newname, callback) {
	...
	// 		'New dataset name cannot be a descendant of current ' +
	// 		dataset name'));
	// 	return;
	// }
	//
